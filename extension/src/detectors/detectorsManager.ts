...
export class DetectorsManager {
  ...
  private resolveServerPath(): string | null {
    ...
    if (configuredPath) {
      this.outputChannel.appendLine(
        `Trying configured path: ${configuredPath}`
      );
      if (this.validateBinary(configuredPath)) {
        return configuredPath;
      }
      this.outputChannel.appendLine(
        `Configured path invalid: ${configuredPath}`
      );
  ...
  private getBundledServerPath(): string | null {
    ...
    const binaryName =
      platform === "win32" ? "language-server.exe" : "language-server";
    ...
    this.outputChannel.appendLine(
      `Binary not found at expected path: ${bundledPath}`
    );
    return null;
  }
  ...
  private handleServerNotFound(): void {
    const message =
      "Language server binary not found. Please install or configure the server path.";
  ...
  private handleScanComplete(scanSummary: ScanSummary) {
    ...
    this.outputChannel.appendLine(
      `Total Rust files: ${scanSummary.total_rust_files}`
    );
    this.outputChannel.appendLine(
      `Anchor programs: ${scanSummary.anchor_program_files}`
    );
    this.outputChannel.appendLine(
      `Files with issues: ${scanSummary.files_with_issues}`
    );
    this.outputChannel.appendLine(
      `Total security issues: ${scanSummary.total_issues}`
    );
    this.outputChannel.appendLine(
      `Anchor.toml files: ${scanSummary.anchor_configs}`
    );
    this.outputChannel.appendLine(
      `Cargo.toml files: ${scanSummary.cargo_files}`
    );
    ...
    if (scanSummary.issues_by_file.length > 0) {
      ...
      scanSummary.issues_by_file.forEach((file) => {
        ...
        this.outputChannel.appendLine(
          `${fileType}${testFlag} ${file.path}: ${file.issue_count} issues`
        );
      });
    }
  ...
  async scanWorkspace(): Promise<void> {
    ...
    try {
      ...
      this.outputChannel.appendLine(
        `Scan request result: ${JSON.stringify(result)}`
      );
    } catch (error) {
  ...
  async reloadDetectors(): Promise<void> {
    ...
    try {
      ...
      this.outputChannel.appendLine(
        `Reload detectors result: ${JSON.stringify(result)}`
      );
    } catch (error) {
  ...
  // Method to run dylint lints on the workspace
  async runDylintLints(): Promise<void> {
    if (!this.client) {
      window.showErrorMessage("Language server not running");
      return;
    }
    this.outputChannel.appendLine("Running dylint lints on workspace...");

    try {
      const result = await this.client.sendRequest("workspace/executeCommand", {
        command: "solana.runDylintLints",
        arguments: [],
      });

      this.outputChannel.appendLine(
        `Dylint lints result: ${JSON.stringify(result)}`
      );
      ...
      if (result && typeof result === "object" && "success" in result) {
        ...
        if (response.success) {
          window.showInformationMessage(
            `Dylint analysis complete: ${
              response.total_diagnostics || 0
            } issues found in ${response.total_files || 0} files`
          );
        } else {
          window.showErrorMessage(
            `Dylint analysis failed: ${response.error || "Unknown error"}`
          );
        }
      }
    } catch (error) {
      this.outputChannel.appendLine(`Error running dylint lints: ${error}`);
      window.showErrorMessage(`Failed to run dylint lints: ${error}`);
    }
  }
}
